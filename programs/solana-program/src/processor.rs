...
// Solana SDK SlotHashes Sysvar Processors (Using Manual Access again to pass
// tests)
...
mod tests {
    ...
    fn test_ping_log_account() {
        ...
        let mut lamports = 0;
        let mut data = vec![];
        let accounts = vec![create_mock_account_info(
            &dummy_key,
            &owner,
            &mut lamports,
            &mut data,
    ...
    fn test_slot_hashes_processing() {
        ...
        let account_info =
            create_mock_account_info(&key, &owner, &mut lamports, &mut data, false, false, false);
        ...
        assert_eq!(
            process_slot_hashes_get_hash_interpolated(&empty_accounts),
            Ok(())
        );
        assert_eq!(
            process_slot_hashes_position_interpolated(&empty_accounts),
            Ok(())
        );
        assert_eq!(
            process_slot_hashes_get_hash_midpoint(&empty_accounts),
            Ok(())
        );
        assert_eq!(
            process_slot_hashes_position_midpoint(&empty_accounts),
            Ok(())
        );
        ...
        assert_eq!(
            process_slot_hashes_get_entry(&wrong_key_accounts),
